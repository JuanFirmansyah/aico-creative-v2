...
const navMenu = [
{name:"Projects",link:"#project-1",icon:"📁"},
{name:"About",link:"#about",icon:"👤"},
{name:"Reel",link:"#reel",icon:"🎬"},
...
Draggable.create(floatingRef.current,{
type:"x,y",
edgeResistance:0.85,
bounds:window
})
...
gsap.to(radialRef.current,{scale:1,opacity:1,duration:0.3})
gsap.to(socialRef.current,{scale:1,opacity:1,duration:0.3})

}

function closeMenu(){

gsap.to(radialRef.current,{scale:0,opacity:0,duration:0.25})
gsap.to(socialRef.current,{scale:0,opacity:0,duration:0.25})
...
{projects.map(p=>(
<li key={p.id}>
<a href={`#${p.id}`} className="hover:text-white">
{p.name}
</a>
</li>
))}
...
className="fixed bottom-10 right-10 w-16 h-16 bg-white text-black rounded-full flex items-center justify-center text-xl z-50 shadow-lg cursor-grab"
>
☰
</div>
...
const angle=i*(360/navMenu.length)
const radius=110
...
className="absolute w-12 h-12 bg-zinc-800 rounded-full flex items-center justify-center text-white pointer-events-auto"
>
{item.icon}
</a>
...
const angle=i*(360/socialMenu.length)
const radius=170
const x=Math.cos(angle*Math.PI/180)*radius
const y=Math.sin(angle*Math.PI/180)*radius
...
href={item.link}
style={{transform:`translate(${x}px,${y}px)`}}
className="absolute w-10 h-10 bg-zinc-700 rounded-full flex items-center justify-center text-white pointer-events-auto"
>
{item.icon}
</a>
...
<a href={p.detail} className="hover:text-white">
{p.name}
</a>
</div>
